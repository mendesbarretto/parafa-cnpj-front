...
export const metadata = { title: "Empresas por cidade e estado | Parafa CNPJ", description: "Encontre empresas por município e estado e consulte seus dados cadastrais.", alternates: { canonical: "/cidades" } };
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-6xl px-5 py-12">
        ...
        <div className="mt-10 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {Object.entries(states).map(([state, stateCities]) => <section key={state}><h2 className="border-b border-border pb-2 text-xl font-bold text-foreground">{state}</h2><div className="divide-y divide-border">{stateCities.map((city) => <a key={city.id} href={cityPath(city)} className="block py-3 text-sm hover:text-primary">{city.name}</a>)}</div></section>)}
        </div>
