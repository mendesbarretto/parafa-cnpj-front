    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-6xl px-5 py-12">
        ...
        <div className="mt-10 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {Object.entries(states).map(([state, stateCities]) => <section key={state}><h2 className="border-b border-border pb-2 text-xl font-bold text-foreground">{state}</h2><div className="divide-y divide-border">{stateCities.map((city) => <a key={city.id} href={`/${city.url ? `${city.url}-${state.toLowerCase()}` : `${city.name.toLowerCase()}-${state.toLowerCase()}`}`} className="flex justify-between py-3 text-sm hover:text-primary"><span>{city.name}</span><span className="text-muted-foreground">{(city.companies_count ?? 0).toLocaleString("pt-BR")}</span></a>)}</div></section>)}
        </div>
